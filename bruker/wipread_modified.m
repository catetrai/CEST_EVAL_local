...
try
    ...
    acqppath = fullfile(directory_Mz, 'acqp');
    acqppathM0 = fullfile(directory_M0, 'acqp');
    recopath = fullfile(directory_Mz, 'pdata','1','reco');
    dicompath = fullfile(directory_Mz, 'pdata','1','dicom','MRIm01.dcm');
...
try
    ...
    P.SEQ.B1                = mineMetaDataFile(methodpath, 'PVM_MagTransPower');
    P.SEQ.n                 = mineMetaDataFile(methodpath, 'PVM_MagTransPulsNumb');
    P.SEQ.tp                = mineMetaDataFile(methodpath, 'PVM_MagTransPulse1')/1000;  % pulse duration in seconds
    P.SEQ.DC                = mineMetaDataFile(acqppath, 'HDDUTY');  % homodecoupling duty cycle? (CHECK)
    P.SEQ.recovertime       = mineMetaDataFile(acqppath, 'ACQ_recov_time');
    P.SEQ.recovertimeM0     = mineMetaDataFile(acqppathM0, 'ACQ_recov_time');
    P.SEQ.sampling          = sampling{1};    % let's assume regular sampling (CHECK)
    P.SEQ.w                 = mineMetaDataFile(acqppath, 'ACQ_O2_list')'/P.SEQ.FREQ;
    P.SEQ.Offset            = max(P.SEQ.w);
    P.SEQ.stack_dim         = [mineMetaDataFile(recopath, 'RECO_size'), 1, P.SEQ.measurements];
